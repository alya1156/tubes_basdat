...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'verify') {
        ...
        $pdo->prepare("UPDATE pembayaran SET status = 'batal' WHERE id_pembayaran = ?")->execute([$id]);
        redirectWithMessage('/tubes_basdat/modules/pembayaran/list.php', 'Pembayaran ditolak', 'success');
    }
}
    ...
    <div class="main-content">
        ...
        <div class="card p-4">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    <div class="info-box">
                        ...
                        <div class="value-info"><span class="badge bg-<?php echo $pembayaran['status'] === 'lunas' ? 'success' : ($pembayaran['status'] === 'pending' ? 'warning' : 'secondary'); ?>"><?php echo ucfirst($pembayaran['status']); ?></span></div>
                    </div>
                </div>
            </div>
            ...
            <?php if ($pembayaran['catatan']): ?>
                <div class="info-box">
                    <div class="label-info">Catatan</div>
            ...
            <?php if ($pembayaran['status'] === 'pending' || $pembayaran['status'] === 'verifikasi'): ?>
                <form method="POST" class="mt-4">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="submit" name="action" value="reject" class="btn btn-danger" onclick="return confirm('Tolak pembayaran ini?')">
                            <i class="bi bi-x-circle"></i> Tolak
                        </button>
                    </div>
                </form>
            <?php else: ?>
                <div class="alert alert-info">Pembayaran sudah diproses</div>
            <?php endif; ?>
        </div>
    </div>
